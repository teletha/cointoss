...
class PositionManagerTest {
    ...
    @BeforeEach
    void init() {
        latest = Variable.of(Executions.BASE);
    ...
    void hasPosition() {
        ...
        positions.add(Executing.buy(1).price(10));
    ...
    void isShort() {
        ...
        positions.add(Executing.sell(1).price(10));
        assert positions.isLong() == false;
    ...
    void size() {
        ...
        positions.add(Executing.buy(1).price(20));
        assert positions.size.is(Num.THREE);

        // short
        positions.add(Executing.sell(2).price(10));
        assert positions.size.is(Num.ONE);

        // turn over
        positions.add(Executing.sell(2).price(20));
        assert positions.size.is(Num.ONE);
    }
    ...
    void price() {
        ...
        // different price long
        positions.add(Executing.buy(2).price(20));
    ...
    @Test
    void profit() {
        latest.set(Executing.buy(1).price(20));
    ...
    void zero() {
        positions.add(Executing.buy(1).price(10));
        positions.add(Executing.sell(1).price(12));
    ...
    @Test
    void increase() {
        positions.add(Executing.sell(3).price(10));
    ...
    @Test
    void decrease() {
        positions.add(Executing.buy(3).price(10));
        assert positions.size.is(Num.of(3));
        positions.add(Executing.sell(1).price(10));
        assert positions.size.is(Num.of(2));
        positions.add(Executing.sell(1).price(10));
    ...
    void added() {
        ...
        // difference price
        positions.add(Executing.buy(1).price(30));
        assert added.size() == 2;

        // exit partially
        positions.add(Executing.sell(2).price(40));
        assert added.size() == 2;

        // exit overflow
        positions.add(Executing.sell(2).price(50));
        assert added.size() == 3;
    }
