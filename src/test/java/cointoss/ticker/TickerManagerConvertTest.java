...
public class TickerManagerConvertTest {
    ...
    void build() {
        ...
        manager.convert(start, end, false).to();
    ...
    void rebuild() {
        ...
        // clear memory cache and rebuild ticker data from same fast log
        ticks.clear();
        manager.convert(start, end, true).to();
        newTicks = ticks.query(start, end).toList();
        assert same(oldTicks, newTicks);
    ...
    void outOfRange() {
        ...
        // build inaide only
        manager.convert(insideStart, insideEnd, false).to();
