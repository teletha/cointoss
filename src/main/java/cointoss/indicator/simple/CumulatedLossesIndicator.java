 ...
 */
public class CumulatedLossesIndicator extends AbstractPeriodicalDelegationIndicator {
     ...
     * @param period
     */
    public CumulatedLossesIndicator(Indicator<Amount> indicator, int period) {
        ...
        calculator = index -> {
            ...
            for (int i = Math.max(1, index - period + 1); i <= index; i++) {
                ...
                if (current.isGreaterThan(prev)) {
                    sumOfLosses = sumOfLosses.plus(current.minus(prev));
                }
            }
            return sumOfLosses;
        };
