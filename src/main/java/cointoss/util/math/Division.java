...
class Division {
    ...
    /**
     * Divides the array 'a' by the array 'b' and gets the quotient and the remainder. Implements
     * the Knuth's division algorithm. See D. Knuth, The Art of Computer Programming, vol. 2. Steps
     * D1-D8 correspond the steps in the algorithm description.
     *
    ...
    static int[] divide(int quot[], int quotLength, int a[], int aLength, int b[], int bLength) {
        ...
        /*
         * Step D1: normalize a and b and put the results to a1 and b1 the normalized divisor's
         * first digit must be >= 2^31
         */
            ...
            if (normA[j] == firstDivisorDigit) {
                ...
                if (guessDigit != 0) {
                    ...
                    do {
                        ...
                        /*
                         * rightHand can overflow; in this case the loop condition will be true in
                        ...
                        /*
                         * checks that longR does not fit in an unsigned int; this ensures that
                         * rightHand will overflow unsigned long in the next step
                         */
    ...
    /**
     * Divides an array by an integer value. Implements the Knuth's division algorithm. See D.
    ...
    static int divideArrayByInt(int dest[], int src[], final int srcLength, final int divisor) {
        ...
        for (int i = srcLength - 1; i >= 0; i--) {
            ...
            if (temp >= 0) {
                ...
                /*
                 * make the dividend positive shifting it right by 1 bit then get the quotient an
                 * remainder and correct them properly
                 */
                long aPos = temp >>> 1;
                long bPos = divisor >>> 1;
                quot = aPos / bPos;
    ...
    /**
     * Performs modular exponentiation using the Montgomery Reduction. It requires that all
     * parameters be positive and the modulus be even. Based <i>The square and multiply algorithm
     * and the Montgomery Reduction C. K. Koc - Montgomery Reduction with Even Modulus</i>. The
     * square and multiply algorithm and the Montgomery Reduction.
     *
    ...
    static BigInteger gcdBinary(BigInteger op1, BigInteger op2) {
        ...
        /*
         * Divide both number the maximal possible times by 2 without rounding gcd(2*a, 2*b) = 2 *
         * gcd(a,b)
         */
    ...
    /**
     * Performs modular exponentiation using the Montgomery Reduction. It requires that all
     * parameters be positive and the modulus be odd. >
     *
     * @see BigInteger#modPow(BigInteger, BigInteger)
    ...
    static BigInteger pow2ModPow(BigInteger base, BigInteger exponent, int j) {
        ...
        BigInteger res2;
        /*
         * If 'base' is odd then it's coprime with 2^j and phi(2^j) = 2^(j-1); so we can reduce
         * reduce the exponent (mod 2^(j-1)).
         */
        if (base.testBit(0)) {
    ...
    /*
     * Implements the Montgomery modular exponentiation based in <i>The sliding windows algorithm
     * and the MongomeryReduction</i>.
    ...
    private static boolean isPowerOfTwo(BigInteger bi, int exp) {
        ...
        result = (exp >> 5 == bi.numberLength - 1) && (bi.digits[bi.numberLength - 1] == 1 << (exp & 31));
        if (result) {
