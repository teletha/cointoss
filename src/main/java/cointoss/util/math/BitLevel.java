...
class BitLevel {
    ...
    static void inplaceShiftLeft(BigInteger val, int count) {
        ...
        val.numberLength += intCount + (Integer.numberOfLeadingZeros(val.digits[val.numberLength - 1]) - (count & 31) >= 0 ? 0 : 1);
     ...
     *            The size of this array is {@code srcLen + 1}, and the format is the same as
    ...
    /**
     * Performs a fast bit testing for positive numbers. The bit to to be tested must be in the
     * range {@code [0, val.bitLength()-1]}
     */
