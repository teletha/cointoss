 ...
 * <p>
 * Since the class was modeled to offer all the functionality as the {@link Integer} class does, it
 * provides even methods that operate bitwise on a two's complement representation of large
...
public class BigInteger extends Number implements Comparable<BigInteger>, Serializable {
    ...
    /**
     * Converts an integral double to an unsigned integer; ie 2^31 will be returned as 0x80000000.
     *
     * @param val
     * @return val as an unsigned int
     */
    private static int toUnsignedInt(double val) {
        return (int) val;
    }
     ...
     * represented as [ 13 ] {@code 2^32 +
     * 13} is represented as [ 13, 1 ] {@code 2^64 + 13} is represented as [ 13, 0, 1 ] {@code 2^31}
     * is represented as [ Integer.MIN_VALUE ] The magnitude array may be longer than strictly
     * necessary, which results in additional trailing zeros.
     * <p>
     * TODO(jat): consider changing to 24-bit integers for better performance in browsers.
     */
    protected transient int digits[];

    /**
     * The length of this in measured in ints. Can be less than digits.length().
     */
    protected transient int numberLength;
     ...
     * significant byte is the entry at index 0. The most significant bit of this entry determines
     * the sign of the new {@code
     ...
     *
     * @param signum sign of the new {@code BigInteger} (-1 for negative, 0 for zero, 1 for
     *            positive).
     * @param magnitude magnitude of the new {@code BigInteger} with the most significant byte
     *            first.
    ...
    /**
     * Constructs a new {@code BigInteger} instance from the string representation. The string
     * representation consists of an optional minus sign followed by a non-empty sequence of digits
     * in the specified radix. For the conversion the method {@code Character.digit(char, radix)} is
     * used.
     *
     * @param val string representation of the new {@code BigInteger}.
     ...
     * @throws NullPointerException if {@code val == null}.
     * @throws NumberFormatException if {@code val} is not a valid representation of a
     *             {@code BigInteger} or if {@code radix < Character.MIN_RADIX} or
    ...
    /**
     * Creates a new {@code BigInteger} whose value is equal to the specified {@code double} (which
    ...
    /**
     * Returns a new {@code BigInteger} whose value is {@code this & ~val}. Evaluating
     * {@code x.andNot(val)} returns the same result as {@code
     * x.and(val.not())}.
     * <p>
     * <b>Implementation Note:</b> Usage of this method is not recommended as the current
     * implementation is not efficient.
     ...
     * <p>
     * Returns the number of bits in the binary representation of {@code this} which differ from the
     * sign bit. If {@code this} is positive the result is equivalent to the number of bits set in
     * the binary representation of {@code this}. If {@code this} is negative the result is
     * equivalent to the number of bits set in the binary representation of {@code -this-1}.
     * <p>
     * <b>Implementation Note:</b> Usage of this method is not recommended as the current
     * implementation is not efficient.
     *
     * @return number of bits in the binary representation of {@code this} which differ from the
     *         sign bit
     */
    public int bitCount() {
     ...
     * <p>
     * The two's complement representation of {@code this} will be at least {@code bitLength() + 1}
     * bits long.
     * <p>
     * The value will fit into an {@code int} if {@code bitLength() < 32} or into a {@code long} if
     * {@code bitLength() < 64}.
     *
     * @return the length of the minimal two's complement representation for {@code this} without
     *         the sign bit.
     */
    ...
    /**
     * Converts value of this {@code BigInteger} to a {@code byte} if it fits it, otherwise
     * {@code ArithmeticException} is thrown.
     *
     * @return this {@code BigInteger} converted to a {@code byte}.
     * @throws ArithmeticException if the value of this {@code BigInteger} does not fit in a
     *             {@code byte}.
     */
    ...
    /**
     * Compares this {@code BigInteger} with {@code val}. Returns one of the three values 1, 0, or
     * -1.
     *
    ...
    /**
     * Returns a {@code BigInteger} array which contains {@code this / divisor} at index 0 and
     * {@code this % divisor} at index 1.
     *
     * @param divisor value by which {@code this} is divided.
     ...
     * represented as an double, then {@code Double.POSITIVE_INFINITY} or
     * {@code Double.NEGATIVE_INFINITY} is returned. Note, that not all integers x in the range
     * [-Double.MAX_VALUE, Double.MAX_VALUE] can be represented as a double. The double
     * representation has a mantissa of length 53. For example, 2^53+1 = 9007199254740993 is
     * returned as double 9007199254740992.0.
     *
    ...
    /**
     * Returns this {@code BigInteger} as an float value. If {@code this} is too big to be
     * represented as an float, then {@code Float.POSITIVE_INFINITY} or
     * {@code Float.NEGATIVE_INFINITY} is returned. Note, that not all integers x in the range
     * [-Float.MAX_VALUE, Float.MAX_VALUE] can be represented as a float. The float representation
     ...
     * {@code val}. If {@code this==0} and {@code val==0} then zero is returned, otherwise the
     * result is positive.
     *
    ...
    /**
     * Converts value of this {@code BigInteger} to an {@code int} if it fits it, otherwise
     * {@code ArithmeticException} is thrown.
     *
     * @return this {@code BigInteger} converted to an {@code int}.
     * @throws ArithmeticException if the value of this {@code BigInteger} does not fit in an
     *             {@code int}.
     */
     ...
     * this is prime with a probability beyond (1-1/2^certainty). If {@code false} is returned, then
     * this is definitely composite. If the argument {@code certainty} <= 0, then this method
     * returns true.
     *
     * @param certainty tolerated primality uncertainty.
    ...
    /**
     * Converts value of this {@code BigInteger} to a {@code long} if it fits it, otherwise
     * {@code ArithmeticException} is thrown.
     *
    ...
    /**
     * Returns a new {@code BigInteger} whose value is {@code this mod m}. The modulus {@code m}
     * must be positive. The result is guaranteed to be in the interval {@code [0, m)} (0 inclusive,
     * m exclusive). The behavior of this function is not equivalent to the behavior of the %
     * operator defined for the built-in {@code int}'s.
     *
     * @param m the modulus.
     ...
     * must be positive. The result is guaranteed to be in the interval {@code [0, m)} (0 inclusive,
     * m exclusive). If {@code this} is not relatively prime to m, then an exception is thrown.
     *
     * @param m the modulus.
     ...
     * inclusive, m exclusive). If the exponent is negative, then
     * {@code this.modInverse(m)^(-exponent) mod m)} is computed. The inverse of this only exists if
     * {@code this} is relatively prime to m, otherwise an exception is thrown.
     *
     ...
     * instance. The probability that the returned {@code
    ...
    /**
     * Returns a new {@code BigInteger} whose value is {@code ~this}. The result of this operation
     * is {@code -this-1}.
     * <p>
     * <b>Implementation Note:</b> Usage of this method is not recommended as the current
     * implementation is not efficient.
     ...
     * methods has the same behavior as the % operator on int's, i.e. the sign of the remainder is
     * the same as the sign of this.
     *
     * @param divisor value by which {@code this} is divided.
     * @return {@code this % divisor}.
     * @throws NullPointerException if {@code divisor == null}.
    ...
    /**
     * Returns a new {@code BigInteger} whose value is {@code this << n}. The result is equivalent
     * to {@code this * 2^n} if n >= 0. The shift distance may be negative which means that
     * {@code this} is shifted right. The result then corresponds to {@code floor(this / 2^(-n))}.
     * <p>
     * <b>Implementation Note:</b> Usage of this method on negative values is not recommended as the
    ...
    /**
     * Returns a new {@code BigInteger} whose value is {@code this >> n}. For negative arguments,
     * the result is also negative. The shift distance may be negative which means that {@code this}
     * is shifted left.
     * <p>
     * <b>Implementation Note:</b> Usage of this method on negative values is not recommended as the
     * current implementation is not efficient.
    ...
    /**
     * Converts value of this {@code BigInteger} to a {@code short} if it fits it, otherwise
     * {@code ArithmeticException} is thrown.
     *
    ...
    /**
     * Tests whether the bit at position n in {@code this} is set. The result is equivalent to
     * {@code this & (2^n) != 0}.
     * <p>
     * <b>Implementation Note:</b> Usage of this method is not recommended as the current
     * implementation is not efficient.
    ...
    public byte[] toByteArray() {
        ...
        /*
         * Puts the little-endian int array representing the magnitude of this BigInteger into the
     ...
     * BigInteger} with base radix. If {@code radix} is less than {@link Character#MIN_RADIX} or
     * greater than {@link Character#MAX_RADIX} then a decimal representation is returned. The
     * characters of the string representation are generated with method {@link Character#forDigit}.
     *
