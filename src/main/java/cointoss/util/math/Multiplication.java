...
class Multiplication {
    ...
    /**
     * Performs the multiplication with the Karatsuba's algorithm. <b>Karatsuba's algorithm:</b>
     * <tt>
    ...
    static BigInteger karatsuba(BigInteger op1, BigInteger op2) {
        ...
        /*
         * Karatsuba: u = u1*B + u0 v = v1*B + v0 u*v = (u1*v1)*B^2 + ((u1-u0)*(v0-v1) + u1*v1 +
         * u0*v0)*B + u0*v0
         */
    ...
    static BigInteger multiplyByTenPow(BigInteger val, int exp) {
        // PRE: exp >= 0
        return ((exp < tenPows.length) ? multiplyByPositiveInt(val, tenPows[exp]) : val.multiply(powerOf10(exp)));
    }
    ...
    /**
     * It calculates a power of ten, which exponent could be out of 32-bit range. Note that
     * internally this method will be used in the worst case with an exponent equals to:
     * {@code Integer.MAX_VALUE - Integer.MIN_VALUE}.
     * 
    ...
    static BigInteger powerOf10(double exp) {
        ...
        // "LARGE POWERS"
        /*
         * To check if there is free memory to allocate a BigInteger of the estimated size, measured
         * in bytes: 1 + [exp / log10(2)]
         */
        if (exp > 1000000) {
        ...
        /*
         * "HUGE POWERS" This branch probably won't be executed since the power of ten is too big.
         */
        // To calculate: 5^exp
