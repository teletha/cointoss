...
class Elementary {
    ...
    static void completeInPlaceSubtract(BigInteger op1, BigInteger op2) {
        ...
        if (op1.sign == 0) {
            ...
            int sign = unsignedArraysCompare(op1.digits, op2.digits, op1.numberLength, op2.numberLength);
            if (sign > 0) {
                subtract(op1.digits, op1.digits, op1.numberLength, op2.digits, op2.numberLength); // op1
                                                                                                  // =
                                                                                                  // op1
                                                                                                  // -
                                                                                                  // op2
                ...
                inverseSubtract(op1.digits, op1.digits, op1.numberLength, op2.digits, op2.numberLength); // op1
                                                                                                         // =
                                                                                                         // op2
                                                                                                         // -
                                                                                                         // op1
    ...
    /**
     * Performs {@code op1 -= op2}. {@code op1} must have enough place to store the result (i.e.
     * {@code op1.bitLength() >= op2.bitLength()}). Both should be positive (what implies that
     * {@code op1 >= op2}).
     * 
     * @param op1 the input minuend, and the output result.
    ...
    static BigInteger subtract(BigInteger op1, BigInteger op2) {
        ...
        if (cmp == BigInteger.LESS) {
            resSign = -op2Sign;
            resDigits = (op1Sign == op2Sign) ? subtract(op2.digits, op2Len, op1.digits, op1Len)
    ...
    /**
     * Compares two arrays, representing unsigned integer in little-endian order. Returns +1,0,-1 if
     * a is - respective - greater, equal or lesser then b
     */
