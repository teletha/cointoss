...
/**
 * Time series compression algorithm described in "www.vldb.org/pvldb/vol8/p1816-teller.pdf". It was
 * produced by translating from
 * "https://github.com/facebookincubator/beringei/tree/master/beringei/lib/{BitUtil,TimeSeriesStream,TimeSeriesStream-inl,}"
 * to Java.
 * <P>
 * Minor changes: initial timestamp takes 8 bytes, a byte buffer instead of folly::fbstring,
 * uint64_t -> long, uint32_t -> int.
 */
public final class CompressedTimeSeries {
    ...
    private void appendValue(double value) {
        ...
        if (leadingZeros >= previousLeadingZeros && trailingZeros >= previousTrailingZeros && previousBlockInformationSize < expectedSize) {
            ...
            write(blockSize - BLOCK_SIZE_ADJUSTMENT, BLOCK_SIZE_LENGTH_BITS); // To fit in 6 bits.
                                                                              // There will never be
                                                                              // a zero size block
    ...
    public long readNextTimestamp() {
        ...
        if (type > 0) { // Delta of delta is non zero. Calculate the new delta. `index` will be used
                        // to find the right length for the value that is read.
            final int index = type - 1;
            long decodedValue = read(TIMESTAMP_ENCODINGS[index].bitsForValue);

            decodedValue -= ((long) 1 << (TIMESTAMP_ENCODINGS[index].bitsForValue - 1)); // [0,255]
                                                                                         // becomes
                                                                                         // [-128,127]
            if (decodedValue >= 0) {
