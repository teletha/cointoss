...
import static io.fair_acc.chartfx.ui.ProfilerInfoBox.DebugLevel.*;
 ...
 * This abstract class assemblies and configures important chart components and elements for
 * financial charts.
...
public abstract class AbstractBasicFinancialApplication {
    ...
    protected ToolBar getTestToolBar(Chart chart, AbstractFinancialRenderer<?> renderer, boolean replaySupport) {
        ...
        if (replaySupport) {
            testVariableToolBar.getItems()
                    .addAll(localRange, periodicTimer, updatePeriod, new Label("[multiply]"), spacer, profilerInfoBox);
        } else {
    ...
    public Node getChartPanel(Stage stage) {
        ...
        Arrays.stream(FinancialTheme.values()).map(this::getDefaultFinancialTestChart).forEach(root.getChildren()::add);
        return root;
    }
    ...
    protected Chart getDefaultFinancialTestChart(final FinancialTheme theme) {
        ...
        if (resource.startsWith("REALTIME")) {
            try {
                ...
                ohlcvDataSet = new SimpleOhlcvReplayDataSet(DataInput.valueOf(resource
                        .substring("REALTIME-".length())), period, timeRangeInt, ttInt, replayFromCal, consolidationAddons);
