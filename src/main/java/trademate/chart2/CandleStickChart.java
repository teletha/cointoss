 ...
 * security, derivative, or currency over time. The Data Y value is used for the opening price and
 * then the close, high and low values are stored in the Data's extra value property using a
...
public class CandleStickChart extends XYChart<String, Number> {
    ...
    public void addBar(BarData bar) {
        ...
        String label = sdf.format(bar.getDateTime());
     ...
     */
    public void updateLast(Num price) {
        if (lastBar != null) {
            lastBar.update(price);
            logger.log(Level.INFO, "Updating last bar with date/time: {0}", lastBar.getDateTime());
            ...
            logger.log(Level.INFO, "Updating last bar with formatteddate/time: {0}", dataSeries.get(0)
                    .getData()
                    .get(datalength - 1)
                    .getXValue());
        }
    ...
    protected void updateAxisRange() {
        // For candle stick chart we need to override this method as we need to let the axis know
